...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  user_id TEXT UNIQUE NOT NULL, -- Clerk user ID from the JWT `sub` claim
...
CREATE TABLE IF NOT EXISTS public.tasks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id TEXT NOT NULL, -- Clerk user ID from the JWT `sub` claim
  title TEXT NOT NULL,
  description TEXT,
...
BEGIN
  ...
  CASE TG_TABLE_NAME
    WHEN 'tasks' THEN entity_type_value := 'task';
    WHEN 'habits' THEN entity_type_value := 'habit';
    WHEN 'events' THEN entity_type_value := 'event';
    ELSE
      RAISE EXCEPTION 'Unsupported table for log_activity: %', TG_TABLE_NAME;
  END CASE;
